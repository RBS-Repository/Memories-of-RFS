// Footer.js
import React from 'react';
import './nicepage.css'
import './index.css';  // Assuming you have a CSS file for styling

const Footer = () => {
  return (
  
  <>
 <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <meta charSet="utf-8" />
  <meta name="keywords" content="​About Us, About Us" />
  <meta name="description" content="" />
  <title>Home</title>
  <link rel="stylesheet" href="nicepage.css" media="screen" />
  <link rel="stylesheet" href="index.css" media="screen" />
  <meta name="generator" content="Nicepage 6.21.2, nicepage.com" />
  <meta name="referrer" content="origin" />
  <link
    id="u-theme-google-font"
    rel="stylesheet"
    href="https://fonts.googleapis.com/css?family=Roboto:100,100i,300,300i,400,400i,500,500i,700,700i,900,900i|Open+Sans:300,300i,400,400i,500,500i,600,600i,700,700i,800,800i"
  />
  <meta name="theme-color" content="#478ac9" />
  <meta property="og:title" content="Home" />
  <meta property="og:type" content="website" />
  <meta data-intl-tel-input-cdn-path="intlTelInput/" />
  <footer
    className="u-align-center u-clearfix u-footer u-gradient u-footer"
    id="sec-5f41"



    


  >   

 {/*Change the FEATURES text here!!! Below*/}



    <div className="custom-expanded u-clearfix u-group-elements u-group-elements-1">
      <h2 className="u-align-center u-text u-text-1"> Our Features</h2>
      <p className="u-align-left u-text u-text-2"> 
        
        
        
        {/*Column text on the right Below*/}

        <span style={{ fontWeight: 700 }}>Welcoming Atmosphere</span>: A
        comfortable, inviting space to work, relax, or catch up with friends.




         {/*Colum text on the right Below*/}

        <span style={{ fontWeight: 700 }}><br></br>Eco-Friendly Practices</span>: We’re
        committed to sustainability with recyclable cups, locally sourced
        ingredients, and eco-friendly practices."
      </p>
      <p className="u-align-left u-text u-text-3">





       {/*Colum text on the left Below*/}

        <span style={{ fontWeight: 700 }}>Freshly Roasted Beans</span>: We
        source, roast, and grind our beans daily to deliver fresh and flavorful
        coffee every time.
        <br />

        {/*Colum text on the left Below*/}

        <span style={{ fontWeight: 700 }}>Variety of Brews</span>: From rich
        espressos to velvety lattes, explore a world of coffee styles and
        flavors.


        <br />
        <span style={{ fontWeight: 700 }} />
        <br />
      </p>
    </div>
    <div className="custom-expanded data-layout-selected u-clearfix u-gutter-0 u-layout-wrap u-layout-wrap-1">
      <div className="u-layout">
        <div className="u-layout-row">
          <div className="u-align-center u-container-align-center u-container-style u-layout-cell u-left-cell u-size-23 u-layout-cell-1">
            <div className="u-container-layout u-container-layout-1">
              <div
                alt=""
                className="u-image u-image-circle u-image-1"
                src=""
                data-image-width={1200}
                data-image-height={1200}
              />
            </div>
          </div>
          <div className="u-align-left u-container-style u-layout-cell u-right-cell u-size-37 u-layout-cell-2">
            <div className="u-container-layout u-container-layout-2">



                {/*Change About us Texy here!!!!!*/}




              <h2 className="u-text u-text-4">About Us</h2>
              <p className="u-text u-text-5">
                {" "}
                Welcome to our coffee shop – where every cup tells a story.
                We’re more than just a coffee shop; we’re a community hub where
                friends, family, and coffee lovers gather to enjoy rich flavors,
                cozy ambiance, and great company. Sourcing our beans from
                ethical farms around the world, we bring you the finest quality
                coffee, brewed with passion and served with a smile. Come in,
                relax, and let us serve you a perfect cup of comfort.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="custom-expanded u-form u-form-1">
    <form
  action="https://formspree.io/f/xgveklqj"
  method="POST"
  className="u-clearfix u-form-spacing-30 u-form-vertical u-inner-form"
  style={{ padding: 10 }}
>
  <div className="u-form-email u-form-group u-form-partition-factor-2">
    <label
      htmlFor="email-319a"
      className="u-label u-text-body-alt-color u-label-1"
    >
      Email
    </label>
    <input
      type="email"
      placeholder="Enter a valid email address"
      id="email-319a"
      name="email"
      className="u-border-2 u-border-no-left u-border-no-right u-border-no-top u-border-white u-input u-input-rectangle"
      required
    />
  </div>
  <div className="u-form-group u-form-name u-form-partition-factor-2">
    <label
      htmlFor="name-319a"
      className="u-label u-text-body-alt-color u-label-2"
    >
      Name
    </label>
    <input
      type="text"
      placeholder="Enter your Name"
      id="name-319a"
      name="name"
      className="u-border-2 u-border-no-left u-border-no-right u-border-no-top u-border-white u-input u-input-rectangle"
      required
    />
  </div>
  <div className="u-form-address u-form-group u-form-group-3">
    <label
      htmlFor="address-452f"
      className="u-label u-text-body-alt-color u-label-3"
    >
      Address
    </label>
    <input
      type="text"
      placeholder="Enter your address"
      id="address-452f"
      name="address"
      className="u-border-2 u-border-no-left u-border-no-right u-border-no-top u-border-white u-input u-input-rectangle"
      required
    />
  </div>
  <div className="u-form-group u-form-message">
    <label
      htmlFor="message-319a"
      className="u-label u-text-body-alt-color u-label-4"
    >
      Message
    </label>
    <textarea
      placeholder="Enter your message"
      rows="4"
      cols="50"
      id="message-319a"
      name="message"
      className="u-border-2 u-border-no-left u-border-no-right u-border-no-top u-border-white u-input u-input-rectangle"
      required
    ></textarea>
  </div>
  <div className="u-align-left u-form-group u-form-submit">
    <input
      type="submit"
      value="Submit"
      className="u-active-white u-border-none u-btn u-btn-round u-btn-submit u-button-style u-hover-white u-palette-2-base u-radius-10 u-btn-1"
    />
  </div>
  <div className="u-form-send-message u-form-send-success">
    Thank you! Your message has been sent.
  </div>
  <div className="u-form-send-error u-form-send-message">
    Unable to send your message. Please fix errors then try again.
  </div>
</form>

    </div>
    <div className="custom-expanded u-list u-list-1">
      <div className="u-repeater u-repeater-1">
        <div className="u-align-center u-black u-container-align-center u-container-style u-list-item u-opacity u-opacity-70 u-radius-20 u-repeater-item u-shape-round u-list-item-1">
          <div className="u-container-layout u-similar-container u-valign-middle u-container-layout-3">
            <span className="u-icon u-icon-circle u-text-palette-2-base u-icon-1">
              <svg
                className="u-svg-link"
                preserveAspectRatio="xMidYMin slice"
                viewBox="0 0 52 52"
                style={{}}
              >
                <use xlinkHref="#svg-077e" />
              </svg>
              <svg
                className="u-svg-content"
                viewBox="0 0 52 52"
                x="0px"
                y="0px"
                id="svg-077e"
                style={{ enableBackground: "new 0 0 52 52" }}
              >
                <path
                  style={{ fill: "currentColor" }}
                  d="M38.853,5.324L38.853,5.324c-7.098-7.098-18.607-7.098-25.706,0h0
	C6.751,11.72,6.031,23.763,11.459,31L26,52l14.541-21C45.969,23.763,45.249,11.72,38.853,5.324z M26.177,24c-3.314,0-6-2.686-6-6
	s2.686-6,6-6s6,2.686,6,6S29.491,24,26.177,24z"
                />
              </svg>
            </span>

                     {/*Office or shop address change below */}

            <h5 className="u-text u-text-6">our main office</h5>
            <p className="u-text u-text-7">
              SoHo 94 Broadway St New York, NY 1001
            </p>
          </div>
        </div>
        <div className="u-align-center u-black u-container-align-center u-container-style u-list-item u-opacity u-opacity-70 u-radius-20 u-repeater-item u-shape-round u-list-item-2">
          <div className="u-container-layout u-similar-container u-valign-middle u-container-layout-4">
            <span className="u-icon u-icon-circle u-text-palette-2-base u-icon-2">
              <svg
                className="u-svg-link"
                preserveAspectRatio="xMidYMin slice"
                viewBox="0 0 513.64 513.64"
                style={{}}
              >
                <use xlinkHref="#svg-9786" />
              </svg>
              <svg
                className="u-svg-content"
                viewBox="0 0 513.64 513.64"
                x="0px"
                y="0px"
                id="svg-9786"
                style={{ enableBackground: "new 0 0 513.64 513.64" }}
              >
                <g>
                  <g>
                    <path d="M499.66,376.96l-71.68-71.68c-25.6-25.6-69.12-15.359-79.36,17.92c-7.68,23.041-33.28,35.841-56.32,30.72    c-51.2-12.8-120.32-79.36-133.12-133.12c-7.68-23.041,7.68-48.641,30.72-56.32c33.28-10.24,43.52-53.76,17.92-79.36l-71.68-71.68    c-20.48-17.92-51.2-17.92-69.12,0l-48.64,48.64c-48.64,51.2,5.12,186.88,125.44,307.2c120.32,120.32,256,176.641,307.2,125.44    l48.64-48.64C517.581,425.6,517.581,394.88,499.66,376.96z" />
                  </g>
                </g>
              </svg>
            </span>

         {/*Phone number change below !!!!!!!*/}


            <h5 className="u-text u-text-8">phone number</h5>
            <p className="u-text u-text-9">
              234-9876-5400 <br />
              888-0123-4567 (Toll Free)
            </p>
          </div>
        </div>
        <div className="u-align-center u-black u-container-align-center u-container-style u-list-item u-opacity u-opacity-70 u-radius-20 u-repeater-item u-shape-round u-list-item-3">
          <div className="u-container-layout u-similar-container u-valign-middle u-container-layout-5">
            <span className="u-icon u-icon-circle u-text-palette-2-base u-icon-3">
              <svg
                className="u-svg-link"
                preserveAspectRatio="xMidYMin slice"
                viewBox="0 0 512 512"
                style={{}}
              >
                <use xlinkHref="#svg-d01e" />
              </svg>
              <svg
                className="u-svg-content"
                viewBox="0 0 512 512"
                id="svg-d01e"
              >
                <path d="m201 12.714v184.286h267v-184.286c0-7.022-5.692-12.714-12.714-12.714h-241.572c-7.022 0-12.714 5.692-12.714 12.714zm63.89 33.131h70.271c8.284 0 15 6.716 15 15s-6.716 15-15 15h-70.271c-8.284 0-15-6.716-15-15s6.715-15 15-15zm0 75.142h139.22c8.284 0 15 6.716 15 15s-6.716 15-15 15h-139.22c-8.284 0-15-6.716-15-15s6.715-15 15-15z" />
                <path d="m472 227h-275c-22.091 0-40 17.909-40 40v205c0 22.091 17.909 40 40 40h275c22.091 0 40-17.909 40-40v-205c0-22.091-17.909-40-40-40zm-207.5 217.5h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm80 120h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm80 120h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15zm0-60h-20c-8.284 0-15-6.716-15-15s6.716-15 15-15h20c8.284 0 15 6.716 15 15s-6.716 15-15 15z" />
                <path d="m87 227h-47c-22.091 0-40 17.909-40 40v205c0 22.091 17.909 40 40 40h47c22.091 0 40-17.909 40-40v-205c0-22.091-17.909-40-40-40z" />
              </svg>
            </span>

                     {/*Your emaill add change bellow*/}

            <h5 className="u-text u-text-10">Your email</h5>
            <p className="u-text u-text-11">123123@mail.com</p>
          </div>
        </div>
        <div className="u-align-center u-black u-container-align-center u-container-style u-list-item u-opacity u-opacity-70 u-radius-20 u-repeater-item u-shape-round u-list-item-4">
          <div className="u-container-layout u-similar-container u-valign-middle u-container-layout-6">
            <span className="u-icon u-icon-circle u-text-palette-2-base u-icon-4">
              <svg
                className="u-svg-link"
                preserveAspectRatio="xMidYMin slice"
                viewBox="0 0 512 512"
                style={{}}
              >
                <use xlinkHref="#svg-9f82" />
              </svg>
              <svg
                className="u-svg-content"
                viewBox="0 0 512 512"
                x="0px"
                y="0px"
                id="svg-9f82"
                style={{ enableBackground: "new 0 0 512 512" }}
              >
                <g>
                  <g>
                    <path d="M507.49,101.721L352.211,256L507.49,410.279c2.807-5.867,4.51-12.353,4.51-19.279V121    C512,114.073,510.297,107.588,507.49,101.721z" />
                  </g>
                </g>
                <g>
                  <g>
                    <path d="M467,76H45c-6.927,0-13.412,1.703-19.279,4.51l198.463,197.463c17.548,17.548,46.084,17.548,63.632,0L486.279,80.51    C480.412,77.703,473.927,76,467,76z" />
                  </g>
                </g>
                <g>
                  <g>
                    <path d="M4.51,101.721C1.703,107.588,0,114.073,0,121v270c0,6.927,1.703,13.413,4.51,19.279L159.789,256L4.51,101.721z" />
                  </g>
                </g>
                <g>
                  <g>
                    <path d="M331,277.211l-21.973,21.973c-29.239,29.239-76.816,29.239-106.055,0L181,277.211L25.721,431.49    C31.588,434.297,38.073,436,45,436h422c6.927,0,13.412-1.703,19.279-4.51L331,277.211z" />
                  </g>
                </g>
              </svg>
            </span>
            <h5 className="u-text u-text-12">Email</h5>
            <p className="u-text u-text-13">
              <a
                href="mailto:hello@theme.com"
                className="u-active-none u-border-1 u-border-active-white u-border-hover-white u-border-no-left u-border-no-right u-border-no-top u-border-palette-2-base u-btn u-button-link u-button-style u-hover-none u-none u-text-active-white u-text-hover-white u-text-palette-2-base u-btn-2"
              >
                hello@theme.com
              </a>
            </p>
          </div>
        </div>
      </div>
    </div>
  </footer>


 
</>





  
  );
};

export default Footer;
